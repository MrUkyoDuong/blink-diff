...
// Ukyo added this codes
/**
 * The images are identical (or near identical) but I don't want error to show up
 *
 * @static
 * @property RESULT_SAME
 * @type {int}
 */
BlinkDiff.RESULT_THE_SAME = 411;
BlinkDiff.RESULT_NOT_THE_SAME = 911;
...
BlinkDiff.prototype = {
    ...
    runWithPromise_Ukyo: function () {
        return Promise.denodeify(this.run_Ukyo).call(this);
    },
    ...
    run_Ukyo: function (fn) {
        ...
        promise.then(function () {
            ...
            if (this._imageOutputPath && this._withinOutputLimit(result.code, this._imageOutputLimit)) {
                this._imageOutput.writeImage(this._imageOutputPath, function (err) {
                    if (err) {
                        console.log("Error_BLOCK # 1");
                        //fn(err);  // Ukyo commented out so no error is thrown
                    } else {
                        this.log("Wrote differences to " + this._imageOutputPath);

                        // console.log("At least 1 image is different as expected.");

                        // Ukyo added
                        // I don't want failure since I am just checking to see if walking into wall bounces back
                        // if this block is executed. I know that the picture is different

                        result = {
                            code: BlinkDiff.RESULT_NOT_THE_SAME,   // non-hack: RESULT_DIFFERENT
                        };                                    // non-hack: will throw an error
                ...
                result = {
                    code: BlinkDiff.RESULT_THE_SAME,   // non-hack: RESULT_DIFFERENT
                };

                // console.log("No difference in image has been detected, yet.");   // Ukyo:  enter when image matches
                fn(undefined, result);
            }

        }.bind(this)).then(null, function (err) {
            console.log("Error_BLOCK # 2");
            //console.error(err.stack);
            //fn(err);
        });
    },
